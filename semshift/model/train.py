...
def _cache_config_and_setup_dirs(config,
    ...
    if config.get("grid_search"):
        ...
        if gs_config_file is None:
            gs_config_file = f"{ROOT_DIR}configurations/hyperparameters/_default.json"
...
def _validate_config_params(config,
    ...
    if configuration_type == "domain_transfer":
        primary_keys.extend(["test_data",
                             "run_on_test",
                             "date_boundaries"])
    elif configuration_type == "learning_curve":
        primary_keys.remove("train_data")
        primary_keys.extend(["source_domain",
                             "target_domain",
                             "run_on_test",
                             "memory",
                             "date_boundaries"])
        ...
        primary_keys.extend(["test_data",
                             ...
                             "min_users_per_split",
                             "min_posts"])
    elif configuration_type == "domain_adaptation":
        primary_keys.remove("train_data")
        primary_keys.remove("grid_search")
        primary_keys.remove("grid_search_kwargs")
        primary_keys.extend(["source_data",
                             "target_data",
                             "adaptation_kwargs"])
    preprocessing_keys = [
            "feature_flags",
    ...
    if configuration_type != "domain_adaptation":
    ...
    if configuration_type == "learning_curve":
        for ds in config["source_domain"] + [config["target_domain"]]:
            if ds not in ds_choices:
                raise ValueError(f"Did not recognize dataset selection: {ds}")
    if configuration_type == "domain_adaptation":
        if config["source_data"] not in ds_choices:
            raise ValueError(f"Configuration parameter 'source_data' incorrectly specified. Must be one of {ds_choices}")
        if config["target_data"] not in ds_choices and not config["target_data"].endswith(".txt") and not os.path.isdir(config["target_data"]):
            raise ValueError(f"Configuration parameter 'target_data' incorrectly specified.")
...
def _isolate_appropriate_controls(metadata_df,
    ...
    elif dataset in ["shen2017"]:
        ## Set Random Seed
        np.random.seed(random_state)
        ## Check Target Disorder
        if target_disorder != "depression":
            raise ValueError("This data set only supports depression as a target disorder!")
        ## Min Data Set Size
        MIN_TWEETS = 25
        metadata_df = metadata_df.loc[metadata_df["num_tweets"] >= MIN_TWEETS].copy()
        ## Isolate Each Group
        depression_df = metadata_df.loc[metadata_df["depression"]=="depression"]
        control_df = metadata_df.loc[metadata_df["depression"]=="control"]
        ## Match Based on Number of Posts
        control_sample =  _generate_stratified_sample(x=depression_df["num_tweets"].values,
                                                      y=control_df["num_tweets"].values,
                                                      n=len(depression_df),
                                                      bins=100)
        ## Combine
        matched_data = pd.concat([depression_df,
                                  control_df.iloc[control_sample]]).reset_index(drop=True).copy()
    elif dataset == "smhd":
...
def load_dataset_metadata(dataset,
    ...
    metadata_df = _isolate_appropriate_controls(metadata_df,
                                                ...
                                                random_state=random_state)
    ## Get Absolute Path
    metadata_df["source"] = metadata_df["source"].map(lambda i: i.replace(ROOT_DIR,"./").replace("//","/"))
    metadata_df["source"] = metadata_df["source"].map(lambda i: f"{ROOT_DIR}/{i}").map(os.path.abspath)
    return metadata_df
...
def dump_predictions(predictions,
    ...
    cache_copy = deepcopy(predictions)
    ## Format Predictions
    data_key = list(cache_copy.keys())[0]
    if update_train:
        cache_copy[data_key]["train"] = dict(("{}__{}".format(x[0],x[1]), y) for x, y in cache_copy[data_key]["train"].items())
    if update_dev:
        cache_copy[data_key]["dev"] = dict(("{}__{}".format(x[0],x[1]), y) for x, y in cache_copy[data_key]["dev"].items())
    ## Dump Predictions
    output_file = "{}predictions.tar.gz".format(output_dir)
    if resampled:
        output_file = output_file.replace(".tar.gz",".resampled.tar.gz")
    with gzip.open(output_file, "wt", encoding="utf-8") as the_file:
        json.dump(cache_copy, the_file)
...
def fit_model(config,
    ...
    model = MentalHealthClassifier(target_disorder=config["target_disorder"],
                                   ...
                                   resolution=train_res.get("resolution"),
                                   reference_point=train_res.get("reference_point"),
                                   chunk_n_samples=train_res.get("chunk_n_samples"),
                                   chunk_randomized=train_res.get("chunk_randomized"),
                                   vocab_fit_args=config.get("vocab_fit_args",{}),
                                   )
    ...
    if grid_search:
        ...
        if grid_search_kwargs["dev_frac"] > 0 and dev_labels is not None:
            ...
            gs_dev_files = np.random.choice(dev_files,
                                            size=int(len(dev_files) * grid_search_kwargs["dev_frac"]),
                                            replace=False)
        ...
        results = model.fit_with_grid_search(train_files=train_files,
                                             ...
                                             resampling_iter=resampling_iter)
    else:
        ## Option 2: Fit With Default Hyperparameters
        results = model.fit(train_files=train_files,
                            label_dict=train_labels,
                            return_training_preds=True,
                            resampling_iter=resampling_iter)
    ## Parse Results
    if resampling_iter is None:
        model, train_pred = results
        train_pred_resample = {}
    else:
        model, train_pred, train_pred_resample = results
    ...
    if dev_files is not None:
        dev_pred = model.predict(test_files=dev_files,
                                 ...
                                 resolution=dev_res.get("resolution",None),
                                 reference_point=dev_res.get("reference_point",None),
                                 chunk_n_samples=dev_res.get("chunk_n_samples"),
                                 chunk_randomized=dev_res.get("chunk_randomized"))
        if resampling_iter is not None:
            dev_pred, dev_pred_resample = dev_pred
    ## Compile Results
    predictions = {data_key : {"train":train_pred, "dev":dev_pred}}
    predictions_resample = {data_key: {"train":train_pred_resample, "dev":dev_pred_resample}}
    ## Cache Model (if Desired)
    if cache_model:
        _ = model.dump("{}model.joblib".format(output_dir))
    ## Cache Predictions
    if cache_predictions:
        _ = dump_predictions(predictions,
                             output_dir,
                             train_res.get("resolution",False),
                             dev_res.get("resolution",False),
                             resampled=False)
        if resampling_iter is not None:
            _ = _ = dump_predictions(predictions_resample,
                                     output_dir,
                                     train_res.get("resolution",False),
                                     dev_res.get("resolution",False),
                                     resampled=True)
    ...
    if resampling_iter is None:
        ...
        return predictions, predictions_resample, model

def fit_adaptive_model(config,
                       data_key,
                       source_files,
                       source_labels,
                       target_files,
                       ...
                       target_date_boundaries={},
                       source_samples={},
                       target_samples={},
                       drop_null_source=False,
                       drop_null_target=False,
                       source_res={},
                       target_res={}):
    """
    ...
    model = MentalHealthClassifier(target_disorder=config["target_disorder"],
                                   ...
                                   jobs=config.get("jobs"),
                                   random_state=config.get("random_seed"),
                                   min_date=source_date_boundaries.get("min_date"),
                                   max_date=source_date_boundaries.get("max_date"),
                                   n_samples=source_samples.get("n_samples"),
                                   randomized=source_samples.get("randomized"),
                                   drop_null=drop_null_source,
                                   resolution=source_res.get("resolution",None),
                                   reference_point=source_res.get("reference_point",None),
                                   chunk_n_samples=source_res.get("chunk_n_samples",None),
                                   chunk_randomized=source_res.get("chunk_randomized",False),
                                   vocab_fit_args=config.get("vocab_fit_args",{}),
                                   )
    ## Fit Model
    model, source_train_preds, target_train_preds = model.fit_with_unsupervised_adaptation(
        train_files=source_files,
        label_dict=source_labels,
        target_files=target_files,
        target_label_dict=target_labels,
        target_min_date=target_date_boundaries.get("min_date"),
        target_max_date=target_date_boundaries.get("max_date"),
        target_n_samples=target_samples.get("n_samples"),
        target_randomized=target_samples.get("randomized"),
        target_resolution=target_res.get("resolution",None),
        target_reference_point=target_res.get("reference_point",None),
        target_chunk_n_samples=target_res.get("chunk_n_samples",None),
        target_chunk_randomized=target_res.get("chunk_randomized",False),
        target_for_vocab=adaptation_kwargs.get("target_in_vocab"),
        target_for_features=adaptation_kwargs.get("target_in_features"),
        return_training_preds=True,
        return_target_preds=True,
        hyperparameter_search=hyperparameter_search,
        hyperparameter_search_kwargs=hyperparameter_search_kwargs
    )
    ## Compile Predictions
    predictions = {data_key : {
                                "train":{
                                        "source":source_train_preds,
                                        "target":target_train_preds,
                                        },
                              }
                   }
    ## Cache Model (if Desired)
    if cache_model and output_dir is not None:
        _ = model.dump("{}model.joblib".format(output_dir))
    ## Cache Predictions
    if cache_predictions and output_dir is not None:
        cache_copy = deepcopy(predictions)
        if target_res.get("resolution"):
            cache_copy[data_key]["train"]["source"] = dict(("{}__{}".format(x[0],x[1]), y) for x, y in cache_copy[data_key]["train"]["source"].items())
        if source_res.get("resolution"):
            cache_copy[data_key]["train"]["target"] = dict(("{}__{}".format(x[0],x[1]), y) for x, y in cache_copy[data_key]["train"]["target"].items())
